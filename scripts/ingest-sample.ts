 ...
 *   npx tsx scripts/ingest-sample.ts https://feedback-analyzer.your-account.workers.dev
 */
...
const sampleFeedback = [
	{
		...
		content: 'The Workers AI response times have been really slow lately. Sometimes it takes 5+ seconds to get a response from Llama. This is blocking our production deployment.',
	...
	},
	{
		...
		message: 'We love R2! The S3 compatibility has made migration super smooth. Would be great to have better event notifications though - the current system is a bit limited for our real-time sync needs.',
	},
	{
		source: 'twitter',
	...
	},
	{
		...
		content: 'The billing dashboard is confusing. I can\'t figure out how to see my Workers usage breakdown vs R2 vs D1. Please add better cost attribution.',
	},
	{
		source: 'discord',
		...
		content: 'URGENT: Our auth service using Workers is returning 500 errors after the latest update. This is a P1 for us - affecting all user logins!',
		timestamp: new Date().toISOString(),
	},
	{
		source: 'github',
...
async function ingestSamples(workerUrl: string) {
	...
	for (let i = 0; i < sampleFeedback.length; i++) {
		...
		try {
			...
			if (result.normalized) {
				console.log(`     Sentiment: ${result.normalized.sentiment}, Urgency: ${result.normalized.urgency}, Area: ${result.normalized.product_area}`);
			}
		} catch (error) {
	...
	console.log(`  curl -X POST ${workerUrl}/ask -H "Content-Type: application/json" -d '{"query": "What are the main issues with Workers AI?"}'`);
